...
exports.handleModelWordpaySignin = (request, callback) => {
    const username = request.payload.username
    const password = request.payload.password
    const queryStatment = 'SELECT COUNT(id_user) '+
    'AS check_sign FROM users '+
    'WHERE username = ? AND password = ? ;'
    connection_mysql.connection.query(queryStatment, [ username, password], (errorDB, resultsDB, fieldsDB) => {
        if (errorDB) {
            throw errorDB
...
exports.handleModelWordpayRegister = (request, callback) => {
    const id_user = uuidv4()
    const username = request.payload.username
    const password = request.payload.password
    const queryStatment = 'INSERT INTO `users` '+
    '(`id_user`,`username`,`password`,`created_at`) '+
    'VALUES (?, ?, ?, NOW())'
    connection_mysql.connection.query(queryStatment, [id_user, username, password], (errorDB, resultsDB, fieldsDB) => {
        if (errorDB) {
            throw errorDB
            console.log('ERROR', errorDB)
            callback(true, '')
